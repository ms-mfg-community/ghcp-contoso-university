...
namespace ContosoUniversity.Web.Controllers
{
    public class StudentsController : BaseController
    {
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            var studentsFiltered = students.AsQueryable();

            if (!String.IsNullOrEmpty(searchString))
            {
                studentsFiltered = studentsFiltered.Where(s => s.LastName.Contains(searchString)
                                       || s.FirstMidName.Contains(searchString));
            }

            studentsFiltered = sortOrder switch
            {
                "name_desc" => studentsFiltered.OrderByDescending(s => s.LastName),
                "Date" => studentsFiltered.OrderBy(s => s.EnrollmentDate),
                "date_desc" => studentsFiltered.OrderByDescending(s => s.EnrollmentDate),
                _ => studentsFiltered.OrderBy(s => s.LastName),
            };

            int pageSize = 10;
            return View(await PaginatedList<Student>.CreateAsync(studentsFiltered, pageNumber ?? 1, pageSize));
        }
